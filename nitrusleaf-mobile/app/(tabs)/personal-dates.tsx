...
export default function PersonalDataScreen() {
  ...
  // Garante que os dados exibidos venham sempre do banco (incluindo CPF)
  useEffect(() => {
    if (!user?.id) return;
    ...
    (async () => {
      try {
        ...
        console.warn("Falha ao carregar dados pessoais do banco:", error);
      }
    })();
  ...
  const handleSave = async () => {
    if (!user?.id) {
      Alert.alert(
        "Sessão inválida",
        "É preciso estar logado para salvar os dados."
      );
      return;
    }
