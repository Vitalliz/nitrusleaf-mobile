...
import { useAnalysisListViewModel } from "@/src/viewmodels/analysisViewModel";

export default function AnalysisScreen() {
  const vm = useAnalysisListViewModel();

  if (vm.loading) return <ActivityIndicator />;
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <View style={{ paddingVertical: 8 }}>
            ...
            <Text>N: {item.nitrogen}</Text>
            <Text>K: {item.potassium}</Text>
            <Text>Ca: {item.calcium}</Text>
          </View>
        )}
      />
    </View>
